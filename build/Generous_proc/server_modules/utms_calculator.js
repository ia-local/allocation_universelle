...
function calculateUtmi(log, modelQualityScores, currentExchangeRates) {
    if (!log || !log.prompt || !log.aiResponse || !log.model || !log.processingTimeMs) {
        // console.warn("Missing data for UTMi calculation:", log);
...
function calculateActivityScore(logs) {
    ...
    logs.forEach(log => {
        ...
        totalPromptWords += log.prompt ? log.prompt.split(/\s+/).filter(word => word.length > 0).length : 0;
        totalResponseTokens += log.aiResponse ? log.aiResponse.split(/\s+/).filter(word => word.length > 0).length : 0;
    });
...
// Cette fonction est un placeholder et nécessitera une implémentation DB pour un suivi réel des PI points
function calculateCurrentUTMAndPIPoints(logs) {
    // console.warn("calculateCurrentUTMAndPIPoints est un placeholder, nécessite une DB pour persister les points.");
    let totalUtmi = 0;
    let totalPiPoints = 0; // Points d'Impact
    if (logs && Array.isArray(logs)) {
        logs.forEach(log => {
            if (log.utmiResult && typeof log.utmiResult.utmi === 'number') {
                totalUtmi += log.utmiResult.utmi;
            }
            // Exemple simple de calcul de PI Points: un PI Point par UTMi généré
            totalPiPoints += log.utmiResult ? log.utmiResult.utmi : 0;
        });
    }
...
function calculateUtmiValueInEUR(utmiValue, economicContext) {
    const { ECONOMIC_CONTEXT, UTMI_TO_CURRENCY_VALUE } = COEFFICIENTS;

    let baseValue = utmiValue * UTMI_TO_CURRENCY_VALUE.UTMI_TO_EUR_BASE;

    if (economicContext) {
        // Appliquer des modificateurs basés sur le contexte économique
        if (economicContext.gdpGrowthRate > ECONOMIC_CONTEXT.GDP_GROWTH_THRESHOLD) {
            baseValue *= (1 + ECONOMIC_CONTEXT.GDP_GROWTH_BONUS_MULTIPLIER);
        }
        if (economicContext.inflationRate < ECONOMIC_CONTEXT.INFLATION_THRESHOLD) {
            baseValue *= (1 + ECONOMIC_CONTEXT.INFLATION_BONUS_MULTIPLIER);
        }
    }
...
function calculateDashboardInsights(logs, targetCurrency = 'EUR', economicContext = null, cvStructuredData = null) {
    ...
    logs.forEach(log => {
        const { utmi, estimatedCostUSD, estimatedCostEUR } = calculateUtmi(log, MODEL_QUALITY_SCORES, ratesForTarget);
        totalUtmi += utmi;
        totalEstimatedCostUSD += estimatedCostUSD;
        totalEstimatedCostEUR += estimatedCostEUR;

        const sentiment = analyzeTextForSentiment(log.aiResponse);
        sentimentCounts[sentiment]++;

        const cognitiveAxes = detectCognitiveAxis(log.prompt + " " + log.aiResponse);
        ...
        if (log.prompt) {
            const promptWords = log.prompt.toLowerCase().split(/\s+/);
        ...
        if (log.interactionType === 'chat_message' && log.aiResponse) {
            totalConversationLengthTokens += log.aiResponse.split(/\s+/).length;
        }
        totalPromptWords += log.prompt ? log.prompt.split(/\s+/).filter(word => word.length > 0).length : 0;
        totalResponseTokens += log.aiResponse ? log.aiResponse.split(/\s+/).filter(word => word.length > 0).length : 0;
    });
    ...
    if (cvStructuredData) {
        ...
        cvLevel = getCvLevel(initialCvValue);
    }
    ...
    let monthlyUniversalIncomeEUR = calculateMonthlyUniversalIncome(
        totalUtmi,
        initialCvValue,
        { junior: cvLevel === 'Junior', middle: cvLevel === 'Moyen', senior: cvLevel === 'Senior' }
    );

    // --- NOUVEAU: Gestion de la Trésorerie (simple cumul) ---
    // Dans un système réel, cela nécessiterait un stockage persistant par utilisateur et une agrégation mensuelle.
    // Ici, pour l'affichage, c'est le revenu mensuel calculé pour les données actuelles.
    let treasuryBalanceEUR = monthlyUniversalIncomeEUR;
    ...
    return {
        ...
        estimatedRevenueEUR: {
            fromUSD: parseFloat(calculateUtmiValueInEUR(totalUtmi, economicContext).toFixed(2)),
        },
...
function calculateInitialCvValue(structuredCvData) {
    ...
    if (structuredCvData.experiences && Array.isArray(structuredCvData.experiences)) {
        structuredCvData.experiences.forEach(exp => {
            ...
            if (exp.description && (exp.description.includes("augmenté") || exp.description.includes("réduit") || exp.description.includes("optimisé") || exp.description.includes("amélioré"))) {
 ...
 * @returns {string} Le niveau du CV (Junior, Moyen, Senior).
 */
 ...
 */
function calculateMonthlyUniversalIncome(totalUtmi, cvValueScore, cvLevelData) {
